...
try:
    AIRFLOW_HOSTS = conf.get("scheduler_failover",
                             "scheduler_nodes_in_cluster")
except Exception as e:
    logging.info("Airflow Scheduler Failover Not Present...")
    AIRFLOW_HOSTS = "localhost"
...
for host in AIRFLOW_HOSTS.split(","):
    for dir_id, directory in enumerate(DIRECTORIES_TO_DELETE):
        ...
        cleanup_task = BashOperator(
            task_id=f'airflow_log_cleanup_{host}',
            bash_command=f"""
