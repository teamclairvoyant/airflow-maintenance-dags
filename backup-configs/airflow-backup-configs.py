...
DAG_ID = os.path.basename(__file__).replace(".pyc", "").replace(".py", "")  # airflow-backup-configs
START_DATE = airflow.utils.dates.days_ago(1)
SCHEDULE_INTERVAL = "@daily"        # How often to Run. @daily - Once a day at Midnight
DAG_OWNER_NAME = "operations"       # Who is listed as the owner of this DAG in the Airflow Web Server
ALERT_EMAIL_ADDRESSES = []          # List of email address to send email alerts to if this job fails

BACKUP_FOLDER_DATE_FORMAT = "%Y%m%d%H%M%S"  # Format options: https://www.tutorialspoint.com/python/time_strftime.htm
...
BACKUP_RETENTION_COUNT = 7          # How many backups to retain (not including the one that was just taken)
...
def print_configuration_fn(**context):
    ...
    context["ti"].xcom_push(key="backup_configs.backup_home_directory", value=BACKUP_HOME_DIRECTORY)
    context["ti"].xcom_push(key="backup_configs.backup_directory", value=BACKUP_DIRECTORY)
    context["ti"].xcom_push(key="backup_configs.backup_retention_count", value=BACKUP_RETENTION_COUNT)
...
def create_backup_directory_fn(**context):
    ...
    logging.info("Loading Configurations...")
    BACKUP_DIRECTORY = context["ti"].xcom_pull(task_ids=print_configuration_op.task_id, key='backup_configs.backup_directory')
...
def delete_old_backups_fn(**context):
    ...
    logging.info("Loading Configurations...")
    BACKUP_HOME_DIRECTORY = context["ti"].xcom_pull(task_ids=print_configuration_op.task_id, key='backup_configs.backup_home_directory')
    BACKUP_RETENTION_COUNT = context["ti"].xcom_pull(task_ids=print_configuration_op.task_id, key='backup_configs.backup_retention_count')
    ...
    if BACKUP_RETENTION_COUNT < 0:
        logging.info("Retention is less then 0. Assuming to allow infinite backups. Skipping...")
        return

    backup_folders = [os.path.join(BACKUP_HOME_DIRECTORY, f) for f in os.listdir(BACKUP_HOME_DIRECTORY) if os.path.isdir(os.path.join(BACKUP_HOME_DIRECTORY, f))]
...
def general_backup_fn(**context):
    ...
    execute_shell_cmd("cp -r -n " + str(PATH_TO_BACKUP) + " " + str(BACKUP_DIRECTORY) + (TARGET_DIRECTORY_NAME if TARGET_DIRECTORY_NAME is not None else ""))
...
if BACKUPS_ENABLED.get("log_directory"):
    backup_op = PythonOperator(
        ...
        python_callable=general_backup_fn,
        params={"path_to_backup": conf.get("core", "BASE_LOG_FOLDER"), "target_directory_name": "logs"},
        provide_context=True,
        dag=dag)
    create_backup_directory_op.set_downstream(backup_op)
    backup_op.set_downstream(delete_old_backups_op)
...
if BACKUPS_ENABLED.get("airflow_cfg"):
    backup_op = PythonOperator(
        ...
        python_callable=general_backup_fn,
        params={"path_to_backup": conf.get("core", "AIRFLOW_HOME") + "/airflow.cfg"},
        provide_context=True,
        dag=dag)
    create_backup_directory_op.set_downstream(backup_op)
    backup_op.set_downstream(delete_old_backups_op)
